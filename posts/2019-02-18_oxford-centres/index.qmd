...
code-fold: false
freeze: true
image: /posts/2019-02-18_oxford-centres/gg_century_centres.png
---
...
![](/posts/2019-02-18_oxford-centres//gg_centres_through_time.png){max-width=530px}
...
colleges_of_oxford <- colleges_of_oxford %>%
  ...
  mutate_at(vars(foundation.year:undergraduates), ~list(
    if (class(.x) == "numeric") {
      .x
    } else {
      parse_number(.x)
...
```{r blog-create-college-geometries, eval = FALSE}
...
```{r blog-append-geometry-data, eval=FALSE}
...
```{r 2018-12-29-oxford-centres-15, eval = FALSE}
