...
But they are! There's a subway connecting them, which is [apparently difficult to discover](https://www.sabre-roads.org.uk/forum/viewtopic.php?t=44620). Thankfully, our data source <www.motorwayservices.info> knows they're connected but does suggest it's a footbridge.
...
The Eat-In variable is the most complicarted, interesting and ripe for visualisation. So let's treat it separately. First we'll identify our twin-site restaurants:
...
Let's import those categorisations
...
I've gone and got the coords from Google Maps

```{r}
data_services_info %>% 
  select(name, is_twin) %>% 
  arrange(name) %>% 
  write_csv(quarto_here("services-locations.csv"))
```
...
I'm going to re-use this data a lot - and potentially try to get it added to the TidyTuesday project! So let's get a reprex script together.


```{r}
list.files(quarto_here("service-stations/"), full.names = TRUE) %>% 
  tibble(
    file_path = .
  ) %>% 
  mutate(is_html = str_ends(file_path, ".html")) %>% 
  View()
```
...
Because there aren't any service stations in Northern Irelane I want to work with shapefiles for just the mainland of Great Britain. The ONS provides high quality data from here https://geoportal.statistics.gov.uk/datasets/ons::countries-december-2023-boundaries-uk-bfc-2/about. I'm going to extract the mainland by st_cast("POLYGON") and discount polygons with an area smaller than 2e10 m^2.
...
Now let's rebuild the chart with this in it:

```{r}

lims_x <- list(min = -12.8, max = 1.76 + 0.2)
lims_y <- list(min = 50 - 0.3, max = 59)

gg_services_roadless <- ggplot() +
  geom_rrect(aes(xmin = lims_x$min - 0.4, 
                 xmax = lims_x$max + 0.4, 
                 ymin = lims_y$min - 0.15, 
                 ymax = lims_y$max + 0),
             fill = colour_motorway_blue,
             colour = "white",
             size = 15,
